...
from llm.prompt_builder import detect_readme_quality, _extract_readme_section
from utils.kv_parser import parse_kv, get_str


def build_technical_prompt(context):
    """
    Hybrid technical prompt.

    Structured README:
        Injects How It Works, Scalability, Novelty, Feature Depth, Feasibility
        sections into the prompt so the LLM can use team-written architecture
        descriptions as additional evidence alongside the code signals.

    Unstructured README:
        No README sections injected — evaluation is purely code-signal based.
        This is fair because the same fallback scoring applies regardless of docs.
    """
    ...
    if quality["mode"] == "structured":
        ...
        scalability   = _extract_readme_section(readme, "Scalability", "Scale")
        ...
        feasibility   = _extract_readme_section(readme, "Feasibility")

        # Only include the block if at least one section has content
        if any([how_it_works, scalability, novelty, feature_depth, feasibility]):
            readme_block = (
                f"\nREADME SECTIONS (written by team — use as architecture/depth evidence):\n"
        ...
        # ── FALLBACK PATH ────────────────────────────────────
        # No README sections to extract — code signals are the only evidence
        readme_block = ""
...
architecture_quality_score= (0-10: separation of concerns, layers, how well parts connect)
reasoning= (one specific sentence about this code referencing actual evidence above)"""


def _is_uniform_fail(result):
    """LLM returned same value for all three fields — it didn't read the evidence."""
    vals = [
...
def run_technical_evaluation(context):
    ...
    if files_a == 0 and files_f == 0:
        return {
            ...
            "engineering_complexity_score": 0,
    ...
    # ── Metric-based fallbacks (same logic regardless of README quality) ──
    ...
    if not reasoning or "hard to provide" in reasoning.lower() or len(reasoning) < 15:
        reasoning = (
            f"{total} files, {funcs} functions, avg {avg} lines/fn, "
