...
def run_project_understanding(context):
    ...
    # Check both analyzed and fallback files — repos with only fallback
    # files are still valid and should not hit the "no files" early return
    ...
    # Fallback: only trigger when problem_summary is genuinely missing or trivial.
    # Don't overwrite a valid LLM response just because it's short.
    if not result["problem_summary"] or len(result["problem_summary"]) < 10:
        ...
        result["problem_summary"]   = (
            f"Project with {total} files and {funcs} functions using {tech} stack."
        )
